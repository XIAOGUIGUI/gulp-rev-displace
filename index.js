...
function getPathByDirRep(list, pattern, path) {
    list.forEach(function (dirRule) {
        if (pattern.indexOf(dirRule.dirRX) > -1) {
            let regexKey = new RegExp(dirRule.dirRX, 'g')
            path = _.isFunction(dirRule.dirRpl)  ? dirRule.dirRpl(path) : path.replace(regexKey, closeDirBySep(dirRule.dirRpl))
        }
    })
    return path
}
    ...
    return through.obj(function (file, enc, cb) {
        ...
        for (var key in manifest) {
            ...
            patterns.forEach(function (pattern) {
                ...
                prefixDelim += '])';
                let replacementPath = manifest[key]
                if ( dirReplacements.length ) {
                    replacementPath = getPathByDirRep(dirReplacements, pattern, manifest[key])
                }
                changes.push({
                    regexp: new RegExp( prefixDelim + pattern, 'g' ),
                    patternLength: pattern.length,
                    replacement: '$1' + replacementPath
                });
            });
        }
